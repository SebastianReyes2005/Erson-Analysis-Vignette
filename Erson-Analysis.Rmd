...
# prior to knitting, run the chunks: 11, 14, 18, 23, 26, 29
rmarkdown::render("~/Documents/GitHub/Erson-Analysis/Erson-Analysis.Rmd", envir = globalenv())
```
...
#run this chunk before knitting, it's just to prevent visualizations from bugging out when knitting the document
...
```{r, eval = FALSE}
ex_sample1 <- SingleHPCA(ex_sample1)
```

```{r}
Idents(ex_sample1) <- ex_sample1@meta.data$HPCACellLabels
DimPlot(ex_sample1, reduction = "umap")
```
...
```{r, eval = FALSE}
ex_sample1 <- SingleImmune(ex_sample1)

#This reference dataset results in a very large number of cell populations, so the below code subsets the largest cell populations for the purposes of a legible legend 
ex_sample1imm <- subset(ex_sample1, subset = NeuroCellLabels == "Imm")
...
```{r}
Idents(ex_sample1) <- ex_sample1@meta.data$ImmuneCellLabels

# subset ensures that only cells that have already been identified as immune cells by a more generalized 
