...
# if all necessary objects are not loaded into the environment, clearit and run the chunk below prior to knitting
...
ex_sample7 <- FindCellClusters(ex_sample7)

#for Top5Markers() example
ex_sample8 <- ex_sample7
ex_sample8 <- RunUMAP(ex_sample8, 1:7)
...
```{r, include = FALSE, eval = FALSE}
#run this and the next chunk before knitting, it's just to prevent visualizations from bugging out when knitting the document
ex_sample9 <- ex_sample1
ex_sample9 <- AddMitoRna(ex_sample9)
ex_sample9 <- QCSeurat(ex_sample9, 200, 3000, 25)
ex_sample9 <- NormalizeAndVariable(ex_sample9)
ex_sample9 <- Scale(ex_sample9)
ex_sample9 <- PcaElbow(ex_sample9)
ex_sample9 <- FindCellClusters(ex_sample9, 1:7, 0.5)
ex_sample9 <- RunUMAP(ex_sample9, dims = 1:7)
```
